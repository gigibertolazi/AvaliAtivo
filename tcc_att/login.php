        ...
        <div class="hero-bg lg:w-1/2 flex flex-col justify-center items-center p-10 text-white">
            <div class="max-w-md">
                ...
                <div class="flex justify-center mb-8">
                    <img src="imglogin.jpeg" alt="Ilustração de uma pessoa usando um laptop com ícones de segurança e login flutuando ao redor" class="rounded-lg shadow-xl" />
                </div>
            </div>
        </div>
  <!-- Login Form -->
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email']) && !empty($_POST['senha'])) {
        ...
        $sql = "SELECT matricula, nome, email, senha, tipo_usuario FROM `$table` WHERE email = ? LIMIT 1";
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if ($result && $usuario = mysqli_fetch_assoc($result)) {
                    if (password_verify($senha, $usuario['senha'])) {
                        // Debug - mostrar todas as informações
                        echo "<div style='background: #f8f9fa; border: 1px solid #ddd; padding: 15px; margin: 20px; border-radius: 5px;'>";
                        echo "<h3 style='color: #333;'>Informações de Debug:</h3>";
                        echo "<pre>";
                        echo "Dados do usuário:\n";
                        echo "Matrícula: " . $usuario['matricula'] . "\n";
                        echo "Nome: " . $usuario['nome'] . "\n";
                        echo "Email: " . $usuario['email'] . "\n";
                        echo "Tipo de usuário: " . $usuario['tipo_usuario'] . "\n";
                        echo "\nDados da Sessão:\n";
                        ...
                        // Debug - pausar execução para visualizar os dados
                        die("Script pausado para debug. Tipo de usuário detectado: " . $usuario['tipo_usuario']);
                        ...
                        switch($usuario['tipo_usuario']) {
                            ...
                            case '2':
                                ...
                                break;
                            default:
                                $login_error = 'Tipo de usuário inválido';
                                break;
                        }
                        exit();
                    }
